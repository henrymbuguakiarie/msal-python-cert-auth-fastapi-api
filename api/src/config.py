...
class Settings(BaseSettings):
    ...
    required_scope: str = Field(
        default="access_as_user", description="Required OAuth2 scope for API access"
    )

    # JWKS Configuration
    jwks_uri: str | None = Field(
        default=None, description="Override JWKS URI (auto-constructed if not provided)"
    )

    # Database Configuration
    database_url: str = Field(
        default="sqlite:///./blog_api.db", description="Database connection string"
    )
